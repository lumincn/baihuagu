...
| _（百花内置）_ | 百花 → 任意 MCP 客户端 | 标准 MCP（streamable-http `/mcp` 端点，挂在 `Baihua.Family`）：知识库 / 家庭只读能力，DSH 经 `dsh-mcp-client` 接入（工具名带 `mcp__baihua__` 前缀） | `Baihua.Family:8788/mcp` |
...
- `mcp__baihua__*` — 百花只读数据工具（知识库检索/列表/读笔记、记账汇总、任务列表），统一经 `Baihua.Family` 内置 `/mcp` 端点暴露（`baihua-dsh-plugin` 不再注册数据工具）
