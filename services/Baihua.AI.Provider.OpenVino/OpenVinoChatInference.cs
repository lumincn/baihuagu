...
public class OpenVinoChatInference : ILocalModelInference
{
    ...
    {
        var omsId = ToOmsModelId(NormalizeModelId(modelPath));
        ...
        try
        {
            ...
            if (data.TryGetProperty("data", out var list) && list.ValueKind == JsonValueKind.Array)
            {
                return list.EnumerateArray().Any(e =>
                    e.TryGetProperty("id", out var id) && id.GetString() == omsId);
            }
        }
        catch (Exception ex)
        {
    ...
    {
        var omsId = ToOmsModelId(NormalizeModelId(modelPath));
        var text = await ChatOnceAsync(omsId, message, systemPrompt, history, cancellationToken);
        if (!string.IsNullOrWhiteSpace(text))
            yield return text;
    }

    private async Task<string?> ChatOnceAsync(string omsId, string message, string? systemPrompt,
        List<(string Role, string Content)>? history, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var resp = await client.PostAsJsonAsync(
                ...
                new { model = omsId, messages, max_tokens = 1024, stream = false },
                cts.Token);
    ...
    /// <summary>百花内部模型标识（RunningModelDto.ModelName）→ OVMS 注册模型 id</summary>
    private static string ToOmsModelId(string internalId)
    {
        return internalId.ToLowerInvariant() switch
        {
            "biancang-instruct" => "biancang",
            "qwen2.5-7b" => "qwen2.5",
