...
> 版本：v1.0  
> 日期：2026-05-01  
> 状态：**历史草案（与现状不符，仅作演进记录）**

> ⚠️ **现状修正（2026-08）**：本文 ADR-001 决定"家庭版不拆微服务"，但实际架构已演进为 **3 个独立后端服务 + 1 个 Web**（Baihua.Family 8788 / Baihua.AI 8791 / Baihua.Vault 8790 / Baihua.Web 5177，AI 独立拆分以隔离 GPU 推理与模型管理），数据库为三个独立 SQLite 文件（family.db / vault.db / ai.db），各服务间通过 `Baihua.Core` 共享数据层与服务。最新架构与约定以仓库根 `AGENTS.md` 为准。
